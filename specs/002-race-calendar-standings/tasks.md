...
- [ ] T005 Implement backend HTTP server bootstrap and Chi routing in `backend/cmd/api/main.go` and `backend/internal/api/router.go`
- [ ] T006 [P] Implement Cosmos DB client, containers, and repository interfaces in `backend/internal/storage/cosmos/client.go` and `backend/internal/storage/repository.go`
...
- [ ] T012 Define Helm charts for backend and frontend workloads in `deploy/helm/backend/` and `deploy/helm/frontend/`
- [ ] T013 Configure HTTPS ingress and firewall-ready egress policy templates in `deploy/helm/backend/templates/ingress.yaml` and `deploy/helm/backend/templates/networkpolicy.yaml`
