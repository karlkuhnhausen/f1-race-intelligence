...
// fetchDriverColors fetches team colors from OpenF1 /drivers endpoint and
// enriches the provided drivers map. Non-fatal — if it fails, colors stay empty.
func fetchDriverColors(ctx context.Context, client *http.Client, sessionKey int, drivers map[int]DriverInfo, logger *slog.Logger) {
	url := fmt.Sprintf("%s/drivers?session_key=%d", openF1BaseURL, sessionKey)
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, url, nil)
	if err != nil {
		logger.Warn("analysis: driver colors request build failed", "error", err.Error())
		return
	}
	req.Header.Set("Accept", "application/json")
	...
	resp, err := client.Do(req)
	if err != nil {
		logger.Warn("analysis: driver colors fetch failed", "error", err.Error())
		return
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		logger.Warn("analysis: driver colors unexpected status", "status", resp.StatusCode)
		return
	}

	var rawDrivers []rawDriver
	if err := json.NewDecoder(resp.Body).Decode(&rawDrivers); err != nil {
		logger.Warn("analysis: driver colors decode failed", "error", err.Error())
		return
	}

	for _, rd := range rawDrivers {
		if info, ok := drivers[rd.DriverNumber]; ok {
			info.TeamColor = rd.TeamColor
			drivers[rd.DriverNumber] = info
		}
	}
	logger.Info("analysis: fetched driver colors", "session_key", sessionKey, "count", len(rawDrivers))
}
...
func FetchAllAnalysisData(ctx context.Context, client *http.Client, sessionKey int, drivers map[int]DriverInfo, logger *slog.Logger) (*AnalysisFetchResult, error) {
	...
	// Fetch team colors first (non-fatal)
	fetchDriverColors(ctx, client, sessionKey, drivers, logger)
